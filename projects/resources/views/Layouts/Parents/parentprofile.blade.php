		...
		<div class="profile-widget profile-widget-info">
			<div class="panel-body">
				<div class="col-lg-2 col-sm-2">
					<h4>{{$parent->parent_first_name}}  {{$parent->parent_last_name}}</h4>
					<h6>Parent</h6>
				</div>
			</div>
		</div>
					...
					<div id="prof" class="tab-pane active">
						<section class="panel">
							...
							<div class="panel-body bio-graph-info">
								...
								<div class="row">
									<div class="bio-row">
										<p><span>First Name </span>: {{$parent->parent_first_name}} </p>
									</div>
									<div class="bio-row">
										<p><span>Last Name </span>: {{$parent->parent_last_name}}</p>
									</div>                                              
									<div class="bio-row">
										<p><span>Birthday</span>: {{$parent->parent_dob}}</p>
									</div>
									<div class="bio-row">
										<p><span>Sex </span>: @if ($parent->parent_sex==1) Male @else Female @endif</p>
									</div>
									<div class="bio-row">
										<p><span>Email </span>:{{$parent->user->email}}</p>
									</div>
									<div class="bio-row">
										<p><span>Address </span>: {{$parent->parent_address}}</p>
									</div>
									<div class="bio-row">
										<p><span>Phone </span>:  {{$parent->parent_phone_number}}</p>
									</div>
									<div class="bio-row">
										<p><span>Username </span>: {{$parent->user->name}}</p>
									</div>
...
<script>
	...
	$("#changePassword_form").validate({
		...
		submitHandler: function(form) {
			...
			$.post("{{ route('parentpasswordchange') }}", {id:id,old_password:old_password,password:password,'_token':$('input[name=_token]').val()}, function(data) {
				if(data=="true")
				{
					$("#sucess").fadeIn();
  ...
  // $('#old_password').after('<span class="text-danger"><strong>Oh snap!!</strong>Incorrect old password.</span>');
	...
	$("#register_form").validate({
		...
		submitHandler: function(form) {
			...
			var email=$('#email').val();
			$.post("{{ route('updateinfoparent') }}", {id:id,parent_id:parent_id,first_name:first_name,last_name:last_name,sex:sex,dob:dob,phone_number:phone_number,address:address,name:name,email:email,'_token':$('input[name=_token]').val()}, function(data) {
				$("#sucess").fadeIn();
